clc;clear;close all;
load('C:\Users\GSPARK\Documents\GitHub\article_figure\NE_HPPC.mat')
load("G:\공유 드라이브\GSP_Data\driving_sample\NE_MCT25oC_HPPC25oC_OCV_KENTECH.mat")
...
for k = 1:length(point_idx)
    ...
    for j = 13:15
        yyaxis left
        plot(data(j).t, data(j).V, 'Color','b'); hold on;
        ylim([3.62 4.6])
        yticks(3.6:0.2:4.6)
        % xlim([5.15 5.35]);
        xlim([18540 19260]);
        ...
        yyaxis right
        plot(data(j).t, data(j).I/56.7, 'Color','r');
        ylim([-2.15 1])
        yticks(-2:0.5:1)
        ylabel('Current [C]')
...
% 
% 
% cd('G:\공유 드라이브\Battery Software Lab\Projects\현대자동차 (1) 2025')
